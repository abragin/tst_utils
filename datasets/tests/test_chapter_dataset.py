...
def test_old_max_length_kwarg_raises():
    # The per-side limit was renamed max_length -> max_side_length; the old
    # kwarg must hard-fail with an informative ValueError (no silent shim).
    with pytest.raises(ValueError, match="max_side_length"):
        ChapterDataset(
            test_df,
            tokenizer=tokenizer,
            source_cols=['text_source_opt1'],
...
def test_max_side_length_required():
    # Omitting the per-side limit entirely must also fail clearly.
    with pytest.raises(ValueError, match="max_side_length"):
        ChapterDataset(
            test_df,
            tokenizer=tokenizer,
